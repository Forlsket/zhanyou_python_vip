...
def do_search():
    ...
    if op_value == 'link':
        ...
        print('query_site={query_site}, search_text={search_text}')
        if search_text == '1':
        ...
        if len(txt_list) > 0:
            ...
            showwarning('搜索结果', '未找到{query}对应的电视剧,请尝试使用链接解析')
...
label3.grid(row=2)
#此处的线路一需要将url栏中http://www.12vv.cn/后的url复制进下面的解析栏播放
site = [('线路一', 'http://www.12vv.cn/'),
